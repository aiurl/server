package io.theurl.framework.reflection;
...
public final class TypeHelper {
    ...
    public static Object coerceValue(Class<?> desiredType, Class<?> valueType, Object value) {
        if (desiredType == null) throw new IllegalArgumentException("desiredType is null");
        ...
        if (Collection.class.isAssignableFrom(boxedDesired) || boxedDesired.isArray() || Map.class.isAssignableFrom(boxedDesired)) {
    ...
    private static Object castNumber(Number number, Class<?> targetNumberClass) {
        if (targetNumberClass == Byte.class) return number.byteValue();
        if (targetNumberClass == Short.class) return number.shortValue();
        if (targetNumberClass == Integer.class) return number.intValue();
        if (targetNumberClass == Long.class) return number.longValue();
        if (targetNumberClass == Float.class) return number.floatValue();
        if (targetNumberClass == Double.class) return number.doubleValue();
        if (targetNumberClass == BigDecimal.class) {
            if (number instanceof BigDecimal) return number;
            return BigDecimal.valueOf(number.doubleValue());
        }
        if (targetNumberClass.isInstance(number)) return number;
    ...
    // Extended conversions: date/time, collections, maps, JSON (when Jackson is available)
    private static boolean isDateTimeTarget(Class<?> boxedDesired) {
        return boxedDesired == Date.class || boxedDesired == Instant.class || boxedDesired == LocalDateTime.class
            || boxedDesired == LocalDate.class || boxedDesired == LocalTime.class || boxedDesired == OffsetDateTime.class
        ...
        if (Temporal.class.isAssignableFrom(target) || target == LocalDate.class || target == LocalDateTime.class
            ...
            for (var p : parsers) {
                try {
                    ...
                    if (parsed instanceof Instant) return convertInstantToTarget(target, (Instant) parsed);
                    if (parsed instanceof ZonedDateTime)
                        return convertInstantToTarget(target, ((ZonedDateTime) parsed).toInstant());
                    if (parsed instanceof OffsetDateTime)
                        return convertInstantToTarget(target, ((OffsetDateTime) parsed).toInstant());
                    if (parsed instanceof LocalDateTime ldt) {
                        return convertInstantToTarget(target, ldt.atZone(ZoneId.systemDefault()).toInstant());
                    }
                    if (parsed instanceof LocalDate ld) {
                        return convertInstantToTarget(target, ld.atStartOfDay(ZoneId.systemDefault()).toInstant());
                    }
                    if (parsed instanceof LocalTime lt) {
                        LocalDate now = LocalDate.now();
                        return convertInstantToTarget(target, LocalDateTime.of(now, lt).atZone(ZoneId.systemDefault()).toInstant());
                    }
                } catch (DateTimeParseException ignored) {
                }
            }
    ...
    private static Object convertInstantToTarget(Class<?> target, Instant inst) {
        if (target == Instant.class) return inst;
        if (target == LocalDateTime.class) return LocalDateTime.ofInstant(inst, ZoneId.systemDefault());
        if (target == LocalDate.class) return LocalDateTime.ofInstant(inst, ZoneId.systemDefault()).toLocalDate();
        if (target == LocalTime.class) return LocalDateTime.ofInstant(inst, ZoneId.systemDefault()).toLocalTime();
        if (target == OffsetDateTime.class) return OffsetDateTime.ofInstant(inst, ZoneId.systemDefault());
        if (target == ZonedDateTime.class) return ZonedDateTime.ofInstant(inst, ZoneId.systemDefault());
        if (target == Date.class) return Date.from(inst);
        return null;
    }
    ...
    private static Object convertToCollectionOrMap(Class<?> desiredType, Object value) {
        if (value == null) return null;
        if (desiredType.isInstance(value)) return value;

        if (value instanceof Map) {
            Object conv = tryJacksonConvert(value, desiredType);
            if (conv != null) return conv;
            if (Map.class.isAssignableFrom(desiredType)) return value;
        }

        if (value instanceof Iterable || value.getClass().isArray()) {
            List<Object> src = new ArrayList<>();
            if (value instanceof Iterable) for (Object o : (Iterable<?>) value) src.add(o);
            else {
                int len = Array.getLength(value);
                for (int i = 0; i < len; i++) src.add(Array.get(value, i));
            }
        ...
        if (value instanceof String) {
            ...
            if (s.startsWith("[") || s.startsWith("{")) {
                ...
                if (parsed != null) {
                    if (desiredType.isInstance(parsed)) return parsed;
    ...
    // Reflection-based optional Jackson integration: parse JSON string to Object using ObjectMapper
