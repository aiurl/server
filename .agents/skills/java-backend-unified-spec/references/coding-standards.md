...
- Mapper：`XxxMapper`
- 仓储接口：`XxxRepository`
- 仓储实现：`InMemory或Jdbc或JpsXxxRepository`
- 命令对象：`XxxCommand`
- 查询对象：`XxxQuery`
- 传输对象：`XxxDto`
- 视图对象：`XxxView`
...
简单 Dto / View / Request / Response 中名称非常直观、且没有额外业务语义的字段，可以在类注释已说明上下文时不重复逐字段展开；但只要字段承担状态、金额、权限、风控、外部映射、兼容分支等关键语义，就必须写清楚。
...
@NoArgsConstructor
@AllArgsConstructor
public class OrderCreateResultView {
...
`Request`、`Response`、`Command`、`Query`、`View` 这类纯数据载体对象，默认遵循以下规则：

- 同时需要 getter / setter 的可变数据载体，默认优先使用 `@Data`
- 只需要只读访问的对象，优先使用 `@Getter`
- 对 `Command`、`Dto`、`View`、内部快照载荷、Convert 转换目标等对象，如果主要通过集中装配、不可变表达或一次性构造完成赋值，默认优先考虑 `@Builder`
- 如果 `Command`、`Query`、`View` 等对象在当前项目中需要开放常规 getter / setter 并承担可变数据载体角色，仍然优先使用 `@Data`
...
- 如果对象已经使用`@Data`、`@Getter`或`@Setter`，除非有已知问题存在，否则尽量不要修改为其他类型。
...
BizAssert.notNull(orderEntity, OrderErrorCodes.ORDER_NOT_FOUND, OrderErrorMessages.ORDER_NOT_FOUND);
```
...
- `Assembler` 默认位于 `web / interfaces`，负责 `Request -> Command`、`View -> Response` 等入口层装配
- `convert` 是默认业务层配套能力，不是可有可无的装饰层
- `Entity -> View`、`Entity -> Dto`、内部 `Dto` 快照组装等跨对象转换，默认优先进入 `XxxConvert`
...
public Long saveTimeline(SaveTimelineCommand saveTimelineCommand) {
    ...
    // 创建
    TimelineVersionEntity timelineVersionEntity = buildTimelineVersionEntity(saveTimelineCommand);
    timelineVersionRepo.insert(timelineVersionEntity);

    // 响应
    return timelineVersionEntity.getId();
}
```
...
```java
@Override
public Page<EmployeeEntity> page(String employeeName, String mobile, Integer status, long current, long size) {
    return employeeMapper.selectPage(
        new Page<>(current, size),
        Wrappers.<EmployeeEntity>lambdaQuery()
            .like(StrUtil.isNotBlank(employeeName), EmployeeEntity::getEmployeeName, employeeName)
            .eq(StrUtil.isNotBlank(mobile), EmployeeEntity::getMobile, mobile)
            .eq(ObjectUtil.isNotNull(status), EmployeeEntity::getStatus, status)
...
```java
.like(employeeName != null && !employeeName.isBlank(), EmployeeEntity::getEmployeeName, employeeName)
.eq(mobile != null && !mobile.isBlank(), EmployeeEntity::getMobile, mobile)
.eq(status != null, EmployeeEntity::getStatus, status)
```
...
public boolean update(EmployeeEntity employeeEntity) {
    return employeeMapper.updateById(employeeEntity) > 0;
}
```
...
public void updateEmployee(UpdateEmployeeCommand command) {
    ...
    EmployeeEntity employeeEntity = employeeRepo.findById(command.getEmployeeId());
    BizAssert.notNull(employeeEntity, AccountErrorCodes.EMPLOYEE_NOT_FOUND, AccountErrorMessages.EMPLOYEE_NOT_FOUND);

    // 更新
    employeeConvert.fillForUpdate(command, employeeEntity);
    boolean updated = employeeRepo.update(employeeEntity);
...
```java
if (employeeMapper.updateById(employeeEntity) != 1) {
